...
public class UserController {
	...
	public UserController(){root = null;}
	boolean isEmpty(){if(root == null) return true; else return false;}
	...
	protected UserControllerNode search(String email, String pWord){
		...
		if(isEmpty()){
			...
			if(email.compareTo(node.patientData.getEmail()) == 0){
				...
				node = node.leftNode;
				return search( node.patientData.getEmail(), node.patientData.getPassword());
			}
			else{
				node = node.rightNode;
				return search( node.patientData.getEmail(), node.patientData.getPassword());
